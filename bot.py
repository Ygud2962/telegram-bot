...
SCHEDULE_STRUCTURED = {
    ...
    '9а': {
        'Понедельник': [
            ...
            (6, 'Русская литература', 'Пинчук А.В.'),
            (7, 'Физкультура', 'Вергейчик В.Л.')
        ],
        'Вторник': [
            (1, 'Физика', 'Зубок С.Н.'),
    ...
    '10а': {
        ...
        'Пятница': [
            ...
            (3, 'Белорусский язык/Матем(проф)', 'Брель Ю.В./Коротчикова Л.В.'),
...
def get_current_lesson_info():
    ...
    # Проверка на длинную перемену между 5 и 6 уроками (12:45-12:55)
    if 12 * 60 + 45 <= current_minutes < 12 * 60 + 55:
        minutes_until = (12 * 60 + 55) - current_minutes
        return {
            'status': 'long_break',
            'next_number': 6,
    ...
    # Проверка перемены до следующего урока
    for start_h, start_m, end_h, end_m, num in lesson_intervals:
        start_total = start_h * 60 + start_m
                ...
                if len(lesson) >= 3:
                    ...
                    if teacher_name:
                        ...
                        for teacher in teacher_list:
                            # Удаляем содержимое в скобках и лишние пробелы
...
async def send_substitution_notification(context, teacher_name, substitution_data):
    """Отправляет уведомление учителю о новой замене с обработкой ошибок."""
    ...
    try:
        ...
        logger.warning(f"❌ Учитель {teacher_name_clean} заблокировал бота")
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(
                    chat_id=admin_id,
                    text=f"❌ Учитель '{teacher_name_clean}' заблокировал бота. Уведомление не доставлено.",
                    parse_mode='HTML'
                )
            except Exception as e:
                logger.error(f"Ошибка отправки уведомления админу об ошибке: {e}")
    except (TimedOut, NetworkError) as e:
        logger.warning(f"⚠️ Временная ошибка сети при отправке учителю {teacher_name_clean}: {e}")
        ...
        try:
            ...
            logger.info(f"✅ Уведомление повторно отправлено учителю {teacher_name_clean}")
        except Exception as e2:
            logger.error(f"❌ Повторная отправка не удалась: {e2}")
            for admin_id in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        chat_id=admin_id,
                        text=f"❌ Не удалось отправить уведомление учителю '{teacher_name_clean}' после повторной попытки:\n{str(e2)[:100]}",
                        parse_mode='HTML'
                    )
                except Exception as e3:
                    logger.error(f"Ошибка отправки уведомления админу об ошибке: {e3}")
    except Exception as e:
        error_msg = str(e)
...
async def show_teacher_selection(query, context):
    """Показывает выбор НОВОГО учителя (шаг 4). Предмет и старый учитель уже определены."""
    teachers = list(TEACHER_IDS.keys())
    keyboard = []
    row = []

    # Исключаем текущего учителя этого предмета
...
async def handle_adding_substitution(query, context):
    """Обрабатывает 4 шага добавления замены: дата → класс → урок → новый учитель."""
    ...
    # 📅 ШАГ 1: Выбор даты → автоматически определяем день недели
    ...
    # 🔢 ШАГ 3: Выбор урока → автоматически получаем предмет и старого учителя из расписания
...
async def save_substitution(query, context):
    """Сохраняет замену: предмет остаётся тем же, меняется только учитель."""
    try:
        date = context.user_data['date']
        day = context.user_data['day']
        class_name = context.user_data['class_name']
        lesson_num = context.user_data['lesson']
        subject = context.user_data['subject']  # Предмет НЕ меняется
        old_teacher = context.user_data['old_teacher']
        new_teacher = context.user_data['new_teacher']

        # Сохраняем замену (предмет остаётся тем же)
        db.add_substitution(
            date, day, lesson_num,
            subject, subject,  # Старый и новый предмет — одинаковые
...
async def show_current_lesson(query, context):
    ...
    elif current_info['status'] == 'long_break':
        minutes = current_info['minutes_until']
        next_num = current_info['next_number']
        text = f"⏸️ <b>СЕЙЧАС ДЛИННАЯ ПЕРЕМЕНА</b>\n"
        text += f"📅 Сегодня: <b>{day_name}</b>\n"
...
async def button_handler(update: Update, context: CallbackContext):
    ...
    # 🔑 ОБРАБОТКА ДОБАВЛЕНИЯ ЗАМЕНЫ (УПРОЩЁННАЯ ВЕРСИЯ - 4 ШАГА)
...
async def show_teacher_schedule(query, context):
    ...
    try:
        teacher_name = query.data.replace('teacher_', '').replace('_', ' ')
    ...
    if not teachers_list or teacher_name not in teachers_list:
        keyboard = [[InlineKeyboardButton("🏠 Старт / Главное меню", callback_data='back_to_main')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            "<b>❌ Ошибка: учитель не найден в списке.</b>",
            reply_markup=reply_markup,
            parse_mode='HTML'
        )
        return
    ...
    for day in days_order:
        if day in schedule and schedule[day]:
            ...
            for lesson in sorted_lessons:
                ...
                # Добавляем 🟢 если это текущий урок сегодня
...
async def show_daily_schedule(query, context):
    ...
    if day in day_mapping:
        ...
        # Если запрашиваем сегодняшний день в будущем (например, сейчас 15:00, а уроки закончились),
        # показываем завтрашний день этой же недели
        if days_ahead == 0 and current_weekday < 5:
            current_minutes = datetime.now(pytz.timezone('Europe/Minsk')).hour * 60 + datetime.now(pytz.timezone('Europe/Minsk')).minute
            if current_minutes > 14 * 60 + 45:  # После окончания 7 урока
                days_ahead = 7
...
async def confirm_clear_substitutions(query):
    ...
    await query.edit_message_text(
        f"⚠️ ПОДТВЕРЖДЕНИЕ ОЧИСТКИ ЗАМЕН\n"
...
async def handle_broadcast_time(update: Update, context: CallbackContext):
    ...
    # ИСПРАВЛЕНО: корректное регулярное выражение (точка экранирована, один пробел)
...
async def confirm_broadcast(query, context):
    ...
    for i, (user_id, username, first_name, last_name) in enumerate(users):
        ...
        # Обновляем статус каждые 5 пользователей или в конце
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    # 🔑 СОХРАНЯЕМ ПОЛЬЗОВАТЕЛЯ ПРИ ПОЛУЧЕНИИ ЛЮБОГО СООБЩЕНИЯ
    user = update.effective_user
    db.add_user(
        user_id=user.id,
        username=user.username,
    ...
    for teacher_name, teacher_id in found_mentions:
        try:
            notification = (
                ...
                f"👤 От: {user.full_name}\n"
                f"📅 Время: {datetime.now(pytz.timezone('Europe/Minsk')).strftime('%H:%M %d.%m.%Y')}\n"
                f"💬 Сообщение:\n"
...
async def test_notification(update: Update, context: CallbackContext):
    ...
    if not teacher_id or teacher_id == 0:
        await update.message.reply_text(f"❌ Для учителя '{found_teacher}' не установлен корректный ID.", parse_mode='HTML')
        return
