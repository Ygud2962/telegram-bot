...
def format_teacher_week_text(teacher_name, teacher_schedule):
    """Возвращает текст с расписанием учителя на неделю (все дни)."""
    if not teacher_schedule:
        return f"<b>👨‍🏫 {teacher_name}</b>\n\nНет уроков в расписании."

    lines = [f"<b>👨‍🏫 {teacher_name} – РАСПИСАНИЕ НА НЕДЕЛЮ</b>", "=" * 40]
        ...
        if day in teacher_schedule and teacher_schedule[day]:
            ...
            for lesson in lessons:
                ...
                if 1 <= lesson_num <= 7:
                    ...
                    emoji = f"{lesson_num}. "
                line = f"{emoji} <b>{lesson_time}</b>   <code>{class_name}</code> ➡️ {subject}"
                lines.append(line)
    return "\n".join(lines)
...
async def show_teacher_schedule_by_name(query, context, teacher_name):
    ...
    week_text = format_teacher_week_text(teacher_name, teacher_schedule)
    ...
    for day in days_order:
        if day in teacher_schedule and teacher_schedule[day]:
            keyboard.append([InlineKeyboardButton(day, callback_data=f'teacher_day_{teacher_global_index}_{day}')])

    if not keyboard:
        keyboard.append([InlineKeyboardButton("❌ Нет уроков", callback_data='noop')])
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await safe_edit_message(query, week_text, reply_markup=reply_markup)

async def show_teacher_day_schedule(query, context, teacher_name, day):
    teacher_schedule = get_cached_teacher_schedule(teacher_name)
    if day not in teacher_schedule or not teacher_schedule[day]:
        await query.answer("❌ На этот день уроков нет")
        return

    lessons = teacher_schedule[day]
    lessons.sort(key=lambda x: x['number'])

    text_lines = [f"👨‍🏫 <b>{teacher_name} – {day}</b>", "─" * 20]
    keyboard = []
    ...
    keyboard.append([InlineKeyboardButton("📅 Неделя", callback_data=f'teacher_week_{teacher_global_index}')])
...
async def button_handler(update: Update, context: CallbackContext):
    ...
    # 🤖 ОБРАБОТКА ИИ-ПОМОЩНИКА
        ...
        if not GPT_AVAILABLE:
            await safe_edit_message(
                ...
                "Попросите администратора настроить GROQ_API_KEY.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("◀️ Назад", callback_data='back_to_main')
                ]])
            )
        ...
        await safe_edit_message(
            ...
            "❌ <b>Я работаю только с текстовыми сообщениями</b>\n\n"
    ...
    # ================== РАСПИСАНИЕ УЧИТЕЛЕЙ (ДНИ И НЕДЕЛЯ) ==================
    elif query.data.startswith('teacher_day_'):
        parts = query.data.split('_', 2)
        if len(parts) == 3:
            try:
                teacher_index = int(parts[1])
                day = parts[2]
                teacher_name = ALL_TEACHERS[teacher_index]
                await show_teacher_day_schedule(query, context, teacher_name, day)
            except (ValueError, IndexError):
                await query.answer("❌ Ошибка при открытии дня")
        return

    elif query.data.startswith('teacher_week_'):
        parts = query.data.split('_', 2)
        if len(parts) == 2:
            try:
                teacher_index = int(parts[1])
                # Создаём фиктивный callback, чтобы вызвать show_teacher_schedule
                class FakeQuery:
                    def __init__(self, data, message, from_user):
                        self.data = data
                        self.message = message
                        self.from_user = from_user
                fake_query = FakeQuery(f'teacher_{teacher_index}', query.message, query.from_user)
                await show_teacher_schedule(fake_query, context)
            except (ValueError, IndexError):
    ...
    if query.data == 'back_to_main':
        ...
        return
    elif query.data.startswith('teacher_') and not query.data.startswith('teacher_search_') and not query.data.startswith('teacher_day_') and not query.data.startswith('teacher_week_'):
...
async def show_daily_schedule(query, context):
    ...
    if class_name not in SCHEDULE_STRUCTURED or day not in SCHEDULE_STRUCTURED[class_name]:
...
async def show_searched_teacher_schedule(query, context):
    try:
        ...
        teacher_schedule = get_cached_teacher_schedule(teacher_name)
        # Используем старую функцию для детального вывода, но можно и новую, если нужно
