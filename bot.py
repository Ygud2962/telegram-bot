...
# [Здесь остается неизменным весь словарь SCHEDULE_STRUCTURED из вашего файла]
# Для краткости я не включаю его полностью в ответ, но он должен быть в вашем коде
SCHEDULE_STRUCTURED = {
    '5а': {
        'Понедельник': [
    ...
    # ... остальные классы (скопируйте из вашего файла)
    '11': {
        'Понедельник': [
...
async def show_teacher_selection(query, context, is_old=True):
    ...
    # Фильтруем учителей с некорректным ID для выбора НОВОГО учителя
    if not is_old:
        teachers = [t for t in teachers if TEACHER_IDS.get(t) not in (None, 0)]
    ...
    if is_old:
        ...
        text += (
            ...
            f"<b>👨‍🏫 Выберите НОВОГО учителя (только с активным Telegram):</b>"
        )
...
async def save_substitution(query, context):
    ...
    try:
        ...
        success_message = (
            ...
            f"<i>Замена будет отображена в расписании класса {class_name.upper()} и в списке замен.</i>"
        )
