...
def build_game_launch_url(base_url: str, game_version: str, payload: str) -> str:
    """Собирает URL Mini App с cache-bust и безопасным добавлением payload."""
    sep = '&' if '?' in base_url else '?'
    cache_bust = int(time.time())
    game_base_url = (
        f"{base_url}{sep}v={urllib.parse.quote(game_version)}"
        f"&cb={cache_bust}"
    )
    if payload and len(payload) < 2048:
        return f"{game_base_url}&tgWebAppStartParam={payload}"
...
async def check_maintenance(update: Update, context: CallbackContext,
    ...
    if scope == 'game':
        ...
        kb = [
            [btn(check_btn_text, check_btn_cb)],
...
async def notify_new_news(context, title: str, scope: str = 'bot'):
    ...
    scope_label = NEWS_SCOPE_LABELS.get(news_scope, NEWS_SCOPE_LABELS[NEWS_SCOPE_BOT])
    msg = (
        "📣 <b>Новая новость:</b>\n"
        f"<b>{title}</b>\n"
        f"<i>Раздел: {scope_label}</i>\n\n"
        "👉 Откройте раздел «Новости» в боте."
    )
    kb = [[btn("📰 Открыть новости", f'news_scope_{news_scope}')]]
    sent = failed = 0
...
NEWS_SCOPE_SCHOOL = 'school'
NEWS_SCOPE_BOT = 'bot'
NEWS_SCOPE_LABELS = {
    NEWS_SCOPE_SCHOOL: "🏫 Новости школы",
    NEWS_SCOPE_BOT: "🤖 Новости бота",
}
NEWS_SCOPE_HEADERS = {
    NEWS_SCOPE_SCHOOL: "🏫 <b>НОВОСТИ ШКОЛЫ</b>",
    NEWS_SCOPE_BOT: "🤖 <b>НОВОСТИ БОТА</b>",
}


def normalize_news_scope(scope: str | None, default: str = NEWS_SCOPE_BOT) -> str:
    raw = (scope or default).strip().lower()
    return raw if raw in NEWS_SCOPE_LABELS else default


def other_news_scope(scope: str) -> str:
    return NEWS_SCOPE_SCHOOL if scope == NEWS_SCOPE_BOT else NEWS_SCOPE_BOT


def news_scope_switch_row(active_scope: str) -> list[InlineKeyboardButton]:
    school_label = ("✅ " if active_scope == NEWS_SCOPE_SCHOOL else "") + NEWS_SCOPE_LABELS[NEWS_SCOPE_SCHOOL]
    bot_label = ("✅ " if active_scope == NEWS_SCOPE_BOT else "") + NEWS_SCOPE_LABELS[NEWS_SCOPE_BOT]
    return [
        btn(school_label, f'news_scope_{NEWS_SCOPE_SCHOOL}'),
        btn(bot_label, f'news_scope_{NEWS_SCOPE_BOT}'),
    ]
...
async def menu_news(query, context):
    ...
    kb = [
        [btn(NEWS_SCOPE_LABELS[NEWS_SCOPE_SCHOOL], f'news_scope_{NEWS_SCOPE_SCHOOL}')],
        [btn(NEWS_SCOPE_LABELS[NEWS_SCOPE_BOT], f'news_scope_{NEWS_SCOPE_BOT}')],
    ]
    if await is_bot_admin_async(uid):
        kb.append([btn("📣 Опубликовать в новости школы", f'admin_publish_news_{NEWS_SCOPE_SCHOOL}')])
        kb.append([btn("📣 Опубликовать в новости бота", f'admin_publish_news_{NEWS_SCOPE_BOT}')])
...
async def menu_news_scope(query, context, scope: str, page: int = 0):
    """
    ...
    if not news_list:
        if await is_bot_admin_async(uid):
            kb.append([btn(f"📣 Опубликовать в «{NEWS_SCOPE_LABELS[scope]}»", f'admin_publish_news_{scope}')])
        kb.append([btn("↩️ К разделам новостей", 'menu_news')])
        kb.append(BACK_TO_MAIN[0])
        await safe_edit(query, f"{NEWS_SCOPE_HEADERS[scope]}\n\n📭 <b>Новостей пока нет</b>", kb)
        return

    # Переворачиваем: новые снизу (ближе к полю ввода)
    news_list = list(reversed(news_list))

    for news_id, title, _content, pub_date, _views in news_list:
        pub_str = convert_utc_to_minsk(pub_date)
        short_title = title if len(title) <= 32 else title[:30] + '…'
        kb.append([btn(f"📰 {short_title}  ·  {pub_str}", f'news_full_{scope}_{news_id}_{page}')])

    # Пагинация: ◀️ — более старые (page+1), ▶️ — более новые (page-1)
    nav = []
    if page + 1 < total_pages:
        nav.append(btn("◀️ Старее", f'news_page_{scope}_{page+1}'))
    nav.append(btn(f"{page+1} / {total_pages}", 'noop'))
    if page > 0:
        nav.append(btn("Новее ▶️", f'news_page_{scope}_{page-1}'))
    if len(nav) > 1:
        kb.append(nav)
    ...
    header = (
        f"{NEWS_SCOPE_HEADERS[scope]}\n"
        f"<i>Всего: {total}  ·  Страница {page+1} из {total_pages}</i>"
    )
    await safe_edit(query, header, kb)


async def show_news_full(query, context, news_id: int, scope: str | None = None, page: int | None = None):
    """Открывает полный текст новости в том же сообщении."""
    resolved_scope = normalize_news_scope(scope or context.user_data.get('news_scope'), NEWS_SCOPE_BOT)
    resolved_page = page
    ...
    if not news:
        ...
        if not news:
            ...
            return
        resolved_scope = normalize_news_scope(news.get('category'), NEWS_SCOPE_BOT)
        resolved_page = int(context.user_data.get(f'news_page_{resolved_scope}', 0) or 0)

    pub_str = convert_utc_to_minsk(news['published_at'])
    scope_label = NEWS_SCOPE_LABELS.get(resolved_scope, NEWS_SCOPE_LABELS[NEWS_SCOPE_BOT])
    text = (
        f"📌 <b>{news['title']}</b>\n"
        f"<i>Раздел: {scope_label}</i>\n"
    ...
    kb = []
    is_admin = await is_bot_admin_async(query.from_user.id)
    if is_admin:
        target_scope = other_news_scope(resolved_scope)
        kb.append([
            btn("✏️ Редактировать", f"edit_news_{news_id}"),
            btn("🗑 Удалить", f"del_news_{news_id}")
        ])
        kb.append([btn(f"↪️ Перенести в «{NEWS_SCOPE_LABELS[target_scope]}»", f"move_news_{news_id}_{target_scope}")])

    kb.append([btn("↩️ К новостям", f'news_page_{resolved_scope}_{resolved_page}')])
    kb.append([btn("↩️ К разделам новостей", 'menu_news')])
    kb.append([btn("🏠 Главное меню", 'back_to_main')])
...
async def move_news_scope(query, context, news_id: int, target_scope: str):
    """Переносит новость между разделами «школа/бот»."""
    target = normalize_news_scope(target_scope, NEWS_SCOPE_BOT)
    ok = await asyncio.to_thread(db.update_news_scope, news_id, target)
    if not ok:
        await query.answer("❌ Не удалось перенести новость", show_alert=True)
        return
    context.user_data['news_scope'] = target
    page = int(context.user_data.get(f'news_page_{target}', 0) or 0)
    await query.answer(f"✅ Перенесено в «{NEWS_SCOPE_LABELS[target]}»")
...
async def start_publish_news(query, context, scope: str | None = None):
    ...
    if news_scope:
        ...
        await safe_edit(
            ...
            "Введите <b>заголовок</b> (до 100 символов):",
            kb
        )
        return

    context.user_data.update({'pub_step': 'scope', 'pub_scope': None})
    kb = [
        [btn(NEWS_SCOPE_LABELS[NEWS_SCOPE_SCHOOL], f'pub_news_scope_{NEWS_SCOPE_SCHOOL}')],
        [btn(NEWS_SCOPE_LABELS[NEWS_SCOPE_BOT], f'pub_news_scope_{NEWS_SCOPE_BOT}')],
...
async def handle_publish_news_input(update: Update, context: CallbackContext):
    ...
    if step == 'scope':
        await update.message.reply_text("Выберите раздел кнопками ниже в сообщении бота.")
        return
    ...
    if step == 'title':
        ...
        await update.message.reply_text(
            ...
            f"<i>Раздел: {NEWS_SCOPE_LABELS[news_scope]}</i>\n\n"
            f"Теперь введите <b>текст новости</b> (до 2000 символов):",
...
async def start_edit_news(query, context, news_id: int):
    ...
    context.user_data.update({
        ...
        'edit_old_scope': normalize_news_scope(news.get('category'), NEWS_SCOPE_BOT),
    })
...
async def handle_edit_news_input(update: Update, context: CallbackContext):
    ...
    if step == 'content':
        ...
        old_scope = normalize_news_scope(context.user_data.get('edit_old_scope'), NEWS_SCOPE_BOT)
        success = await asyncio.to_thread(db.update_news, news_id, new_title, text, old_scope)
        _clear_flow(context)
        if success:
            kb = [[btn("📰 К новостям", f'news_scope_{old_scope}')]]
            await update.message.reply_text(
                f"✅ Новость обновлена!\n\n<b>{new_title}</b>",
                parse_mode='HTML', reply_markup=InlineKeyboardMarkup(kb)
            )
...
async def menu_game(query, context):
    ...
    game_url = build_game_launch_url(GAME_URL, GAME_VERSION, payload)
...
async def _button_handler_impl(update: Update, context: CallbackContext):
    ...
    # ── Новости ──
    if d.startswith('news_scope_'):
        scope = d.replace('news_scope_', '', 1)
        await menu_news_scope(query, context, scope, 0)
        return
    if d.startswith('news_page_'):
        ...
        if len(parts) >= 4:
            scope = parts[2]
            page = int(parts[3])
            await menu_news_scope(query, context, scope, page)
        else:
            # Legacy callback support: news_page_<page>
            page = int(parts[2])
            scope = normalize_news_scope(context.user_data.get('news_scope'), NEWS_SCOPE_BOT)
            await menu_news_scope(query, context, scope, page)
        return
    if d.startswith('news_full_'):
        ...
        if len(parts) >= 5:
            ...
            news_id = int(parts[3])
            page = int(parts[4])
            await show_news_full(query, context, news_id, scope, page)
        else:
            # Legacy callback support: news_full_<id>
            news_id = int(parts[2])
            scope = normalize_news_scope(context.user_data.get('news_scope'), NEWS_SCOPE_BOT)
            page = int(context.user_data.get(f'news_page_{scope}', 0) or 0)
            await show_news_full(query, context, news_id, scope, page)
        return
