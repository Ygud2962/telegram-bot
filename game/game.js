...
const SECRET_MODE_META = Object.freeze({
  none:    { title: 'Обычный режим', bonus_pct: 0,  short: 'без бонуса' },
  silent:  { title: 'Тихий шифр', bonus_pct: 20, short: '1 задание без подсказки' },
  speed:   { title: 'Скоростной радист', bonus_pct: 12, short: '3 быстрых ответа подряд' },
  iron:    { title: 'Железная воля', bonus_pct: 16, short: 'ответ с 1 жизнью' },
  recruit: { title: 'Вербовщик', bonus_pct: 10, short: 'пригласи 1 агента' },
  night:   { title: 'Ночной дозор', bonus_pct: 14, short: 'ответ в редкое время' },
});
  ...
  return rawMissions
    ...
    .map((item) => {
      ...
      return {
        ...
        name: String(item.name || 'Тайная миссия'),
...
function _showSecretAwardNotice(award, totalPoints) {
  ...
  el.style.cssText = `
    ...
    transition:opacity .25s ease, transform .25s ease;
  `;
  const missionName = escapeHtml(String(award && award.name ? award.name : 'Тайная миссия'));
  const missionIcon = escapeHtml(String(award && award.icon ? award.icon : '🕶'));
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:flex-start;gap:10px">
      ...
      <div style="min-width:0">
        <div style="font-family:var(--head);font-size:11px;letter-spacing:.09em;color:#8bf4d3">ТАЙНАЯ МИССИЯ ВЫПОЛНЕНА</div>
        <div style="font-family:var(--head);font-size:16px;color:#dcfff4;letter-spacing:.03em;margin-top:2px">${missionName}</div>
        <div style="font-size:12px;color:#c8f0e4;margin-top:4px">Награда: <b>+${points} оч</b> · бонус <b>+${bonusPct}%</b></div>
        <div style="font-size:11px;color:#9ed9c7;margin-top:3px">Суммарный бонус миссий: +${totalBonus} оч</div>
      </div>
    </div>
  `;
...
function showSuccess(cipher, pts, elapsed) {
  ...
  const solvedLabel = cipher.type === 'quiz' ? quizOption : 'Ответ принят';
  ...
  if (factEl) {
    factEl.innerHTML = cipher.fact
      ? '<span style="color:var(--accent3);margin-right:4px">📜</span>' + cipher.fact
      : '';
  }
...
function renderProfileTab() {
  ...
  }).join('') || '<div style="font-size:var(--fs-sm);color:var(--muted);font-style:italic">Список миссий загрузится после первой синхронизации.</div>';
  ...
  el.innerHTML = `
    ...
    <div style="background:#121a17;border:1px solid rgba(103,255,211,.26);border-radius:8px;padding:14px;margin-bottom:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:9px">
        <div style="font-family:var(--head);font-size:var(--fs-xs);color:#8bf4d3;letter-spacing:.09em">🕶 ТАЙНЫЕ МИССИИ</div>
      ...
      <div style="font-size:11px;color:#d9fff2;line-height:1.5;margin-bottom:8px">
        Режим: <b>${secretMeta.title}</b> · бонус режима <b>+${secretMeta.bonus_pct}%</b><br>
        Накопленный бонус миссий: <b>+${secretSummary.bonus_points} оч</b>
      </div>
      ${secretRows}
      <button onclick="switchTab('secret')" style="width:100%;margin-top:10px;background:rgba(103,255,211,.1);border:1px solid rgba(103,255,211,.28);color:#8bf4d3;padding:9px;font-family:var(--head);font-size:11px;border-radius:6px;cursor:pointer;letter-spacing:.06em">ОТКРЫТЬ ЦЕНТР МИССИЙ</button>
    </div>
...
const MAP_CITIES = [
  ...
  { name:'НАРОВЛЯ',  x:245, y:300, hint:'Юго-восток, Припять' },
...
function setSecretMode(modeLike) {
  ...
  if (nextMode === 'none') {
    showPenaltyNotice('Тихий режим выключен. Бонусы тайных миссий не начисляются.', 5200);
  } else {
    showPenaltyNotice(
      `Активирован режим «${meta.title}». Выполни условие и получи бонус +${meta.bonus_pct}% к очкам миссии.`,
      6200
    );
  }
...
const ACHIEVEMENTS = [
  ...
  { id:'secret_first', icon:'🕶️', name:'Тень в эфире', desc:'Выполни 1 тайную миссию.', pts:25 },
  { id:'secret_starter_3', icon:'🧭', name:'Тихий контур', desc:'Выполни 3 стартовые тайные миссии.', pts:45 },
  { id:'secret_starter_5', icon:'🛰️', name:'Режим призрака', desc:'Выполни все 5 стартовых тайных миссий.', pts:75 },
  { id:'secret_advanced_3', icon:'🎖️', name:'Штабная глубина', desc:'Выполни 3 продвинутые тайные миссии.', pts:85 },
  { id:'secret_hard_combo', icon:'⚙️', name:'Тройной протокол', desc:'Выполни «Операция без следов», «Радиоэфир» и «Последний рубеж».', pts:130 },
  { id:'secret_bonus_200', icon:'💼', name:'Закрытый фонд', desc:'Накопи +200 очков бонуса за тайные миссии.', pts:60 },
  { id:'secret_bonus_450', icon:'👑', name:'Секретный резерв', desc:'Накопи +450 очков бонуса за тайные миссии.', pts:110 },
  { id:'secret_all_10', icon:'🛡️', name:'Невидимый фронт', desc:'Выполни все 10 тайных миссий.', pts:160 },
  { id:'secret_all_15', icon:'🗝️', name:'Ключ штаба', desc:'Выполни все 15 тайных миссий.', pts:200, reward:150 },
  { id:'secret_map_runner', icon:'🧭', name:'Маршрут проводника', desc:'Выполни миссию «Маршрут проводника».', pts:75, reward:55 },
  { id:'secret_type_mix', icon:'🧪', name:'Комбинированный канал', desc:'Выполни миссию «Комбинированный канал».', pts:80, reward:60 },
  { id:'secret_clean_two', icon:'📋', name:'Аккуратный штаб', desc:'Выполни миссию «Аккуратный штаб».', pts:90, reward:70 },
  { id:'secret_evening_watch', icon:'🌆', name:'Вечерний дозор', desc:'Выполни миссию «Вечерний дозор».', pts:70, reward:50 },
  { id:'secret_shock_score', icon:'⚡', name:'Штурмовой темп', desc:'Выполни миссию «Штурмовой темп».', pts:95, reward:75 },
  { id:'secret_advanced_6', icon:'🛰️', name:'Глубокая разведка', desc:'Выполни 6 продвинутых тайных миссий.', pts:130, reward:90 },
  { id:'secret_bonus_700', icon:'💰', name:'Резерв операции', desc:'Накопи +700 очков бонуса за тайные миссии.', pts:120, reward:90 },
  { id:'secret_bonus_1000', icon:'🏦', name:'Фонд фронта', desc:'Накопи +1000 очков бонуса за тайные миссии.', pts:160, reward:120 },
  { id:'streak_12', icon:'🌋', name:'Несокрушимая серия', desc:'Серия из 12 правильных ответов.', pts:95, reward:70 },
  { id:'fast_60', icon:'🛩️', name:'Скоростная эскадрилья', desc:'60 быстрых ответов (<=10 сек).', pts:110, reward:85 },
  { id:'agent_5', icon:'🕸️', name:'Куратор сети', desc:'Пригласи 5 агентов.', pts:90, reward:70 },
  ...
  { id:'collector_24', icon:'🧰', name:'Хранитель досье', desc:'Получи 24 достижения', pts:95, reward:75 },
  { id:'collector_32', icon:'🏛️', name:'Мастер архива', desc:'Получи 32 достижения', pts:140, reward:110 },
];
...
const ARTIFACTS = [
  ...
  { id:'artifact_secret_shadow', icon:'🕶️', rarity:'rare', name:'Печать тени', desc:'Получено за первую тайную миссию.' },
  { id:'artifact_secret_modes', icon:'🛰️', rarity:'epic', name:'Пульт режимов', desc:'Выполнены все 5 стартовых тайных миссий.' },
  { id:'artifact_secret_night', icon:'🌙', rarity:'rare', name:'Ночной пропуск', desc:'Выполнена миссия «Ночной дозор».' },
  { id:'artifact_secret_radio', icon:'📻', rarity:'epic', name:'Кодовый эфир', desc:'Выполнена миссия «Радиоэфир».' },
  { id:'artifact_secret_network', icon:'🧩', rarity:'epic', name:'Сетка агентов', desc:'Выполнена миссия «Сеть информаторов».' },
  { id:'artifact_secret_legend', icon:'👑', rarity:'legendary', name:'Архив невидимого фронта', desc:'Выполнены все 10 тайных миссий.' },
  { id:'artifact_secret_map', icon:'🧭', rarity:'rare', name:'Маршрутная линейка', desc:'Выполнена миссия «Маршрут проводника».', reward:45 },
  { id:'artifact_secret_mix', icon:'🧪', rarity:'epic', name:'Полевой дешифратор', desc:'Выполнена миссия «Комбинированный канал».', reward:50 },
  { id:'artifact_secret_clean', icon:'📋', rarity:'epic', name:'Протокол точности', desc:'Выполнена миссия «Аккуратный штаб».', reward:60 },
  { id:'artifact_secret_evening', icon:'🌆', rarity:'rare', name:'Вечерняя сводка', desc:'Выполнена миссия «Вечерний дозор».', reward:40 },
  { id:'artifact_secret_strike', icon:'⚡', rarity:'epic', name:'Штурмовой ордер', desc:'Выполнена миссия «Штурмовой темп».', reward:70 },
  { id:'artifact_secret_elite', icon:'🗝️', rarity:'legendary', name:'Ключ штаба', desc:'Выполнены все 15 тайных миссий.', reward:120 },
  { id:'artifact_secret_bonus_1000', icon:'🏦', rarity:'legendary', name:'Золотой резерв', desc:'Накоплено +1000 очков бонуса тайных миссий.', reward:95 },
  { id:'artifact_agents_hq', icon:'🕸️', rarity:'epic', name:'Штаб агентуры', desc:'В сети 5 активных агентов.', reward:70 },
  { id:'artifact_speed_60', icon:'🛩️', rarity:'epic', name:'Крыло перехвата', desc:'Сделано 60 быстрых ответов.', reward:75 },
  { id:'artifact_collector_24', icon:'🧰', rarity:'epic', name:'Сундук архивариуса', desc:'Получено 24 достижения.', reward:80 },
...
function showAchievementToast(ach, rewardPoints = 0) {
  ...
  el.innerHTML = `
    ...
    <div>
      ...
      ${rewardPoints > 0 ? `<div style="font-size:10px;color:#9dffb7;margin-top:2px">Награда: +${rewardPoints} к общему счёту</div>` : ''}
    </div>`;
  document.body.appendChild(el);
  setTimeout(() => {
...
function renderAchievementsTab(container) {
  ...
  const starterHtml = starterRows.length
    ...
    : '<div style="font-size:10px;color:#9cc7ba;font-style:italic">Стартовые миссии загрузятся после синхронизации.</div>';
  const advancedHtml = advancedRows.length
    ? advancedRows.map(missionRow).join('')
    : '<div style="font-size:10px;color:#9cc7ba;font-style:italic">Продвинутые миссии загрузятся после синхронизации.</div>';
  ...
  let html = `<div style="padding:16px">
    ...
    <div style="background:linear-gradient(160deg,rgba(85,255,210,.12),rgba(12,20,17,.96));
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
        <div>
          <div style="font-family:var(--head);font-size:12px;color:#9df7da;letter-spacing:.08em">🕶 ТАЙНЫЕ МИССИИ</div>
          <div style="font-size:10px;color:#c9f7ea;margin-top:3px">Активный режим: <b>${secretMeta.title}</b> (+${secretMeta.bonus_pct}%)</div>
        </div>
      ...
      <div style="margin-top:6px;font-size:10px;color:#c7efe3;text-align:right;">Бонус миссий: +${secretStats.bonusPoints} оч</div>
      ...
      <div style="margin-top:10px;padding:8px;border-radius:8px;background:rgba(103,255,211,.05);border:1px dashed rgba(103,255,211,.2)">
        <div style="font-family:var(--head);font-size:10px;color:#8ff2d2;letter-spacing:.07em">БЫСТРЫЙ СТАРТ</div>
        ${starterHtml}
      </div>
      <div style="margin-top:10px;padding:8px;border-radius:8px;background:rgba(255,255,255,.03);border:1px dashed rgba(255,255,255,.16)">
        <div style="font-family:var(--head);font-size:10px;color:#b8e0d2;letter-spacing:.07em">НОВЫЙ РЕЖИМ · ОПЕРАЦИИ</div>
        ${advancedHtml}
      </div>
      <div style="margin-top:8px;font-size:10px;color:#9cc7ba;line-height:1.45">
        Антиабуз: серия «Скоростного радиста» сбрасывается при ошибке, подсказке, паузе или выходе из главы.
      </div>
      <button onclick="switchTab('secret')" style="width:100%;margin-top:10px;background:rgba(103,255,211,.1);border:1px solid rgba(103,255,211,.28);color:#8bf4d3;padding:9px;font-family:var(--head);font-size:11px;border-radius:6px;cursor:pointer;letter-spacing:.06em">ОТКРЫТЬ ЭКРАН ТАЙНЫХ МИССИЙ</button>
    </div>
  ...
  ACHIEVEMENTS.forEach(ach => {
    ...
    html += `<div style="background:${has ? 'linear-gradient(170deg,#2b240c,#171208)' : 'linear-gradient(170deg,#1a1711,#100e0a)'};
      ...
      ${Number(ach.reward || 0) > 0 ? `<div style="font-size:9px;color:${has ? '#9dffb7' : '#8eaa8f'};margin-top:4px">Награда: +${Number(ach.reward || 0)} к счёту</div>` : ''}
    </div>`;
  });
  ...
  ARTIFACTS.forEach(artifact => {
    ...
    html += `<div style="background:${has ? 'linear-gradient(168deg,#1f1a12,#13100b)' : 'linear-gradient(168deg,#171510,#0f0d09)'};
      ...
      ${Number(artifact.reward || 0) > 0 ? `<div style="font-size:9px;color:${has ? '#9dffb7' : '#8eaa8f'};margin-top:4px">Награда: +${Number(artifact.reward || 0)} к счёту</div>` : ''}
    </div>`;
  });
...
function renderSecretMissionsTab() {
  ...
  const modeCards = SECRET_MODE_ORDER.map((mode) => {
    ...
    const label = isNone ? 'Обычный' : 'Активировать';
    return `<div style="background:${active ? 'rgba(103,255,211,.14)' : 'rgba(255,255,255,.03)'};
      ...
      <div style="font-size:10px;color:#b7dacd;line-height:1.4;margin-top:4px">${escapeHtml(meta.short || 'без бонуса')}</div>
      <button onclick="setSecretMode('${mode}')" style="width:100%;margin-top:8px;background:${active ? 'rgba(103,255,211,.22)' : 'rgba(255,224,51,.1)'};border:1px solid ${active ? 'rgba(103,255,211,.45)' : 'rgba(255,224,51,.25)'};color:${active ? '#8bf4d3' : '#ffe8a8'};padding:8px;border-radius:6px;font-family:var(--head);font-size:10px;letter-spacing:.06em;cursor:pointer">${active ? 'АКТИВЕН' : label}</button>
    </div>`;
  }).join('');
  ...
  const missionCard = (mission) => {
    ...
    const rewardLine = done
      ? `Награда получена: +${Math.max(0, Number(mission.reward_points || 0))} оч`
      : `Награда: бонус +${Math.max(0, Number(mission.bonus_pct || 0))}%`;
    return `<div style="background:${done ? 'linear-gradient(165deg,rgba(28,55,46,.95),rgba(12,24,20,.96))' : 'linear-gradient(165deg,rgba(25,22,16,.95),rgba(14,12,9,.95))'};
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
        <div style="font-size:20px;line-height:1">${escapeHtml(mission.icon || '🕶️')}</div>
        <div style="flex:1;min-width:0">
          <div style="font-family:var(--head);font-size:11px;color:${done ? '#cffff0' : '#f2e6c3'};letter-spacing:.04em">${escapeHtml(mission.name || 'Тайная миссия')}</div>
        ...
        <div style="text-align:right">
          ...
          <div style="font-size:9px;color:${done ? '#8ff2d2' : '#9b9278'}">${done ? 'ГОТОВО' : 'В ПРОЦЕССЕ'}</div>
        </div>
      </div>
  ...
  el.innerHTML = `
    <div style="padding:18px 14px 22px">
      <div style="background:linear-gradient(160deg,rgba(85,255,210,.16),rgba(11,19,16,.96));
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
          <div>
            <div style="font-family:var(--head);font-size:15px;color:#9df7da;letter-spacing:.08em">🕶 ЦЕНТР ТАЙНЫХ МИССИЙ</div>
            <div style="font-size:11px;color:#c9f7ea;margin-top:4px">Активный режим: <b>${escapeHtml(secretMeta.title)}</b> (+${secretMeta.bonus_pct}%)</div>
          </div>
        ...
        <div style="display:flex;justify-content:space-between;gap:8px;margin-top:8px;font-size:10px;color:#bfeee0">
          <span>Выполнено: <b>${secretStats.completed}</b></span>
          <span>Бонус миссий: <b>+${secretStats.bonusPoints} оч</b></span>
        </div>
      </div>
      ...
      <div style="background:rgba(255,224,51,.05);border:1px solid rgba(255,224,51,.2);border-radius:10px;padding:12px;margin-bottom:12px">
        <div style="font-family:var(--head);font-size:11px;color:#ffe8a8;letter-spacing:.07em;margin-bottom:8px">НОВЫЙ РЕЖИМ ОПЕРАЦИЙ</div>
        <div style="font-size:10px;color:#d5ccad;line-height:1.45;margin-bottom:10px">
          Тайные миссии отличаются от обычных глав: здесь важны стиль прохождения, дисциплина, скорость и тактика.
          Режимы применяются в заданиях глав и дают отдельные бонусы.
        </div>
      ...
      <div style="display:flex;gap:8px;margin-bottom:8px">
        ...
        <div style="flex:1;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.18);border-radius:8px;padding:8px;text-align:center">
          ...
          <div style="font-size:9px;color:#b6ac90">ПРОДВИНУТЫЕ</div>
        </div>
      </div>

      <div style="font-family:var(--head);font-size:10px;color:#8ff2d2;letter-spacing:.07em;margin:10px 0 7px">СТАРТОВЫЕ МИССИИ</div>
      <div style="display:grid;gap:8px">
        ${starterRows.length ? starterRows.map(missionCard).join('') : '<div style="font-size:10px;color:#9cc7ba;font-style:italic">Миссии появятся после синхронизации.</div>'}
      </div>

      <div style="font-family:var(--head);font-size:10px;color:#b8e0d2;letter-spacing:.07em;margin:12px 0 7px">ПРОДВИНУТЫЕ МИССИИ</div>
      <div style="display:grid;gap:8px">
        ${advancedRows.length ? advancedRows.map(missionCard).join('') : '<div style="font-size:10px;color:#9cc7ba;font-style:italic">Миссии появятся после синхронизации.</div>'}
      </div>

      <div style="margin-top:10px;font-size:10px;color:#9cc7ba;line-height:1.45">
        Антиабуз: прогресс скоростных серий обнуляется при ошибке, подсказке, паузе или выходе из главы.
      </div>
      <button onclick="switchTab('chapters')" style="width:100%;margin-top:10px;background:rgba(255,224,51,.15);border:1px solid rgba(255,224,51,.35);color:#ffe8a8;padding:10px;border-radius:7px;font-family:var(--head);font-size:11px;letter-spacing:.07em;cursor:pointer">ПЕРЕЙТИ К ГЛАВАМ</button>
    </div>
  `;
}
...
function renderSettingsTab() {
  ...
  el.innerHTML = `
    <div style="padding:20px 16px">
      ...
      <div class="settings-card">
        <div class="settings-card-title">🕶 ТИХИЙ РЕЖИМ · ТАЙНЫЕ МИССИИ</div>
        <div class="settings-caption" style="margin-top:0">
          Выбери специальный режим. При выполнении условий миссии начисляется бонус к очкам.
        </div>
        ...
        <div style="margin-top:8px;padding:9px 10px;border-radius:8px;background:rgba(103,255,211,.08);border:1px solid rgba(103,255,211,.24)">
          <div style="font-family:var(--head);font-size:11px;color:#8bf4d3;letter-spacing:.07em">АКТИВНЫЙ РЕЖИМ</div>
          <div style="font-size:13px;color:#d5fff3;margin-top:4px">${secretMeta.title} · бонус +${secretMeta.bonus_pct}%</div>
        ...
        <div class="settings-caption" style="margin-top:10px">
          Прогресс тайных миссий: <b>${secretSummary.completed}/${secretSummary.total}</b> · бонус получен: <b>+${secretSummary.bonus_points} оч</b>.
        </div>
        <div class="settings-caption" style="margin-top:6px">
          Антиабуз: серия режима «Скоростной радист» сбрасывается при ошибке, подсказке, паузе или выходе из главы.
        </div>
      </div>

      <div class="settings-card">
        <div class="settings-card-title">🧹 ОЧИСТКА КЕША</div>
