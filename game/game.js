...
function _applySyncResponse(result, forceServerState = false) {
  ...
  if (mergeRes.applied || secretChanged) {
    saveState();
    _refreshCurrentTabAfterSync();
  }
...
const DEFAULT_STATE = () => ({
  ...
  referralAgents: [],
  secretMode: 'none',
  secretSummary: { completed: 0, total: 10, bonus_points: 0 },
  secretMissions: [],
...
function normalizeSecretSummary(rawSummary) {
  ...
  const total = Math.max(1, Math.floor(Number(src.total || 10)));
...
function renderProfileTab() {
  ...
  el.innerHTML = `
    ...
    <div style="background:#121a17;border:1px solid rgba(103,255,211,.26);border-radius:8px;padding:14px;margin-bottom:12px">
      ...
      ${secretRows}
      <button onclick="switchTab('achievements')" style="width:100%;margin-top:10px;background:rgba(103,255,211,.1);border:1px solid rgba(103,255,211,.28);color:#8bf4d3;padding:9px;font-family:var(--head);font-size:11px;border-radius:6px;cursor:pointer;letter-spacing:.06em">ОТКРЫТЬ ВСЕ МИССИИ</button>
    </div>
    ...
    <div style="background:linear-gradient(160deg,rgba(85,255,210,.12),rgba(12,20,17,.96));
      ...
      <div style="display:flex;gap:8px;margin-top:9px">
        <div style="flex:1;background:rgba(103,255,211,.08);border:1px solid rgba(103,255,211,.18);border-radius:7px;padding:7px;text-align:center">
          <div style="font-family:var(--head);font-size:12px;color:#a9ffe5">${secretStats.completedStarter}/5</div>
          <div style="font-size:9px;color:#9ed8c8">СТАРТОВЫЕ</div>
        </div>
        <div style="flex:1;background:rgba(103,255,211,.08);border:1px solid rgba(103,255,211,.18);border-radius:7px;padding:7px;text-align:center">
          <div style="font-family:var(--head);font-size:12px;color:#a9ffe5">${secretStats.completedAdvanced}/5</div>
      ...
      <div style="margin-top:10px;padding:8px;border-radius:8px;background:rgba(103,255,211,.05);border:1px dashed rgba(103,255,211,.2)">
        <div style="font-family:var(--head);font-size:10px;color:#8ff2d2;letter-spacing:.07em">ПЕРВАЯ ВОЛНА (БЕЗ БОЛИ)</div>
        ${starterHtml}
      </div>
      <div style="margin-top:10px;padding:8px;border-radius:8px;background:rgba(255,255,255,.03);border:1px dashed rgba(255,255,255,.16)">
        <div style="font-family:var(--head);font-size:10px;color:#b8e0d2;letter-spacing:.07em">ПРОДВИНУТЫЕ ОПЕРАЦИИ</div>
        ${advancedHtml}
      </div>
