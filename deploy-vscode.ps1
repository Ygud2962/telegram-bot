...
if ($normalModeOk) {
    Write-Host ""
    Write-Host "Deploy complete: pushed to GitHub/main. Not deployed yet -- on the VPS run: git pull && docker compose up -d --build" -ForegroundColor Green
    exit 0
}
...
try {
    ...
    Write-Host ""
    Write-Host "Deploy complete (safe mode): pushed to GitHub/main. Not deployed yet -- on the VPS run: git pull && docker compose up -d --build" -ForegroundColor Green
}
finally {
